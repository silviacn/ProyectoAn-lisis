...
def main():
    ...
    def printSolution(distGraph):
        # este gusrda en un arreglo el resultado del grafo y lo ordena
        string = "inf"

        nodes =distGraph.keys()
        # saca la primera fila las letras de las columnas
        resultado = []
        resultado.append("        ")
        for n in nodes:
        ...
        # termina de graficar el resultado de las ruta mas cortas
    ...
    def floydWarshall(graph):
        ...
        distance = {}
        # Elsiguiente for recorre los nodos
        for n in nodes:

            distance[n] = {}

            for k in nodes:
                # guarda los varlores de la distantcia
                distance[n][k] = graph[n][k]
        # K seria la cantidad de vertices que tiene el grafo
        for k in nodes:

            for i in nodes:

                for j in nodes:
                    # logica para saber el resultado de cual es la ruta optima  o mas corta
